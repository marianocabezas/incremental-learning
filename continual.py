                ...
                if param.requires_grad:
                    if param.grad is not None:
                        ...
                        if (dotp < 0).any():
                            ...
                            param.grad.data.copy_(final_grad)
                    p += 1
...
class IndependentGEM(ParamGEM):
    ...
    def project(self, gradient, memories, eps=1e-3):
        return GEM.project(self, gradient, memories, eps)
...
class DER(IncrementalModelMemory):
    ...
    def auxiliary_loss(self, prediction, target, offset1, offset2):
        if self.current_task > 0:
            target = torch.clamp(
...
class TaskGEM(DyTox, ParamGEM):
    ...
    def mini_batch_loop(self, data, train=True):
        return ParamGEM.mini_batch_loop(self, data, train)
